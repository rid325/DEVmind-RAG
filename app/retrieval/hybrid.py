...
def search_hybrid(db: Session, query: str, k: int = 5, rerank: bool = True, use_hyde: bool = True, enhanced: EnhancedQuery | None = None, timings: dict | None = None) -> list:
    ...
    if timings is not None:
        timings["embedding"] = round((perf_counter() - start) * 1000, 2)
    ...
    if not doc_ids:
        if timings is not None:
            timings["search"] = round((perf_counter() - embedded) * 1000, 2)
    ...
    if timings is not None:
        timings.update(search=round((retrieved-embedded)*1000, 2),
                       reranking=round((end-retrieved)*1000, 2))
